...
if __name__ == '__main__':
    ...
    net.load_state_dict(torch.load('state_1800e_50s_1batch.pth', map_location=torch.device('cpu')))
    test_single(net, 'data/')
    net.load_state_dict(torch.load('state_10000s.pth', map_location=torch.device('cpu')))
    test_single(net, 'data/')
    img.show()
    target.show()

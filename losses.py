...
def LossA(discriminator, device, output_g, target, optim_d, lossT=False, train_disc=True):
    ...
    l_true = torch.full((batch_size,), random.uniform(0.9, 1.0), device=device)
    l_fake = torch.full((batch_size,), random.uniform(0.0, 0.1), device=device)
